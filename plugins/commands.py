...
async def start(client, message):
    try: 
        ...
        if settings.get('fsub_id', AUTH_CHANNEL1) == AUTH_REQ_CHANNEL:
            ...
            id = settings.get('fsub_id', AUTH_CHANNEL1)
            channel = int(id)
            if settings.get('fsub_id', AUTH_CHANNEL1) and not await is_subscribed(client, message.from_user.id, channel):
